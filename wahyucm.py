...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mWahyu_Marpaung_XD')
...
def banner():
	clear()
	wahyu_XD(f'''\t{asu} ______                                              
\t|_   _ \                                             
\t  | |_) | _ .--.  ,--.   ____   ____  .---.  _ .--.  
\t  |  __'.[ `/'`\]`'_\ : [_   ] [_   ]/ /__\\[ `/'`\] 
\t _| |__) || |    // | |, .' /_  .' /_| \__., | |     	
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Wahyu C Marpaung {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
		...
		try:
			wahyuheker = requests.get('https://graph.facebook.com/me?fields=id&access_token='+tokenku[0], cookies={'cookie':cok})
			wahganteng = json.loads(wahyuheker.text)['id']
			menu(wahganteng)
		except KeyError:
			login_lagi334()
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........Jalankan Lagi Perintahnya Kontol!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....CEK TUMBAL LUU NGENTOD !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(id):
	try:
	...
	print('>> 5. Hasil Crack  ')
	print('>> 0. Keluar       ')
	_____wahyu__marpaung_____ = input('\n>> Pilih : ')
	if _____wahyu__marpaung_____ in ['1']:
		dump_massal()
	elif _____wahyu__marpaung_____ in ['2']:
		dump_follower()
	elif _____wahyu__marpaung_____ in ['3']:
		error()
	elif _____wahyu__marpaung_____ in ['4']:
		crack_file()
	elif _____wahyu__marpaung_____ in ['5']:
		result()
	elif _____wahyu__marpaung_____ in ['0']:
		os.system('rm -rf .token.txt')
		os.system('rm -rf .cookie.txt')
		print('>> Sukses Logout+Hapus Kukis ')
		exit()
	else:
		print('>> Pilih Yang Bener Asu Ngentod ')
		back()
def error():
...
def dump_massal():
	...
	try:
		...
		print('>> Masukkan Angka ya Anjing, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
